...
def update_part_full():
    """Update multiple fields of a part (Current, Min, Max, Bin, Bin Checked, Pirana Status, Mark for Delete)"""
    try:
        data = request.json
        ...
        mark_delete_col = headers.index('Mark for Delete') if 'Mark for Delete' in headers else 17
        
        print(f"[UPDATE-FULL] Column indices - Stock Code: {stock_code_col}, Current: {current_col}, Min: {min_col}, Max: {max_col}, Bin: {bin_col}, Bin Checked: {bin_checked_col}, Ready Pirana: {ready_pirana_col}, Added Pirana: {added_pirana_col}, Mark Delete: {mark_delete_col}")
        ...
        # ===== NEW: Mark for Delete support (ONLY CHANGE) =====
        if 'mark_delete' in data:
            cell_range = f'{chr(65 + mark_delete_col)}{row_idx}'
            mark_delete_value = 'TRUE' if data['mark_delete'] in [True, 'true', 'TRUE', '1', 1] else ''
            updates.append({
                'range': cell_range,
                'values': [[mark_delete_value]]
            })
            updated_fields.append('mark_delete')
            print(f"[UPDATE-FULL] Will update Mark for Delete at {cell_range} to {mark_delete_value}")
        # ===== END NEW CODE =====
